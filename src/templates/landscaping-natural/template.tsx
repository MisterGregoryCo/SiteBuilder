...
function LandStats({ stats }: { stats: {value:string;label:string}[] }) {
  ...
  return (<div ref={ref} className={`max-w-5xl mx-auto px-6 -mt-10 relative z-20 transition-all duration-700 ${v?'opacity-100 translate-y-0':'opacity-0 translate-y-6'}`}>
...
function LandCard({ svc, i }: { svc: {name:string;description:string;image?:string}; i: number }) {
  ...
  return (<div ref={ref} className={`group rounded-2xl overflow-hidden shadow-md transition-all duration-700 hover:shadow-xl hover:-translate-y-1 ${v?'opacity-100 translate-y-0':'opacity-0 translate-y-8'}`}
    style={{ background: C.white, transitionDelay: `${i*80}ms` }}>
