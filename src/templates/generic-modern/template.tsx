...
function SectionLabel({ text }: { text: string }) {
  ...
  return (
    <div ref={ref} className={`text-center mb-4 transition-all duration-700 `}>
...
function ServiceCard({ service, index }: { service: { name: string; description: string; image?: string }; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`group relative rounded-2xl overflow-hidden transition-all duration-700 `} style={{transitionDelay: `${index * 100}ms`,
