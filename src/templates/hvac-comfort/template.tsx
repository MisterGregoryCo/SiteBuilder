...
function HvacStats({ stats }: { stats: {value:string;label:string}[] }) {
  ...
  return (<div ref={ref} className={`max-w-5xl mx-auto px-6 -mt-16 relative z-20 transition-all duration-700 `}>
...
function HvacServiceBand({ svc, i }: { svc: {name:string;description:string;image?:string}; i: number }) {
  ...
  const isEven = i % 2 === 0;
  return (
    <div ref={ref} className={`relative rounded-2xl overflow-hidden transition-all duration-700 hover:shadow-xl `} style={{background: isEven ? C.white : C.offWhite, border: `1px solid ${C.border}`, transitionDelay: `${i*80}ms` }}>
...
function HvacContact({ cfg, siteId }: { cfg: Site['site_config']; siteId: string }) {
  ...
  return (<section id="contact" className="py-28 md:py-36" style={{ background: C.offWhite }}>
    <div ref={ref} className={`max-w-5xl mx-auto px-6 lg:px-8 transition-all duration-1000 `}>
      {submitted ? (
